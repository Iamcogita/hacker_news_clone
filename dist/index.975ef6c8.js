...
async function APIfetcher(url) {
    return await fetch(url).then((response)=>response.json()).then((json)=>{
        let news = [];
        json.hits.forEach((e)=>{
            if (e.title !== null) news.push(new newsArticle(e.title, e.url, e.objectID));
        });
        return news;
    });
}
...
function printNews(news) {
    ...
    for (const article of news){
        ...
        button.addEventListener("click", async ()=>{
            ...
            } else document.getElementById(commentID).remove();
        });
...
},{"./model.js":"dEDha"}],"dEDha":[function(require,module,exports) {
class newsArticle {
    constructor(title, url, objectID){
        this.title = title;
        this.url = url;
...
},{}]},["ShInH","8lqZg"], "8lqZg", "parcelRequirefc04")
